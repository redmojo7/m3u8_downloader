...
def main():
    ...
    for url in url_list:
        print(f"\n *** start to download {url_list.index(url)} ***\n")
        ...
        #
        if url_list.index(url) == 15:
            downloader.download(output="kyfsj-pronunciation-" + str(url_list.index(url)) + ".ts")
        #
        # thread = threading.Thread(target=downloader.download, args=("kyfsj-pronunciation-" + str(url_list.index(url)) + ".ts",))
        # thread = threading.Thread(target=thread_function, args=(str(url_list.index(url)),))
        # thread.start()

        print(f"\n *** download end {url_list.index(url)} ***\n")
